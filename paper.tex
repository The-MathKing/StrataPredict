...
% --- Title & Author ---
\title{A Bochner-Weitzenb\"ock Framework for Topologically-Motivated Graph Representation Learning}
...
\begin{abstract}
The 1-dimensional Weisfeiler-Lehman (1-WL) graph isomorphism test fundamentally bounds iterative neighborhood aggregation schemes on discrete graphs. This topological limitation arises because strictly dyadic (pairwise) interactions fail to capture higher-dimensional combinatorial invariants, such as chordless cycles and dense cliques. We introduce \textbf{DynamicCW}, an architectural framework that topologically lifts graphs into 2-dimensional CW complexes and elevates 2-cells (faces) to first-class, learning-active entities with continuous embeddings. Rather than utilizing static topological descriptors, our model dynamically learns cellular representations through cross-dimensional message passing via the absolute boundary incidence matrices $|B_1|$ and $|B_2|$. We show that the 1-WL-indistinguishable $4\times4$ Rook's graph and Shrikhande graph are separated by the Betti numbers of their clique complexes, $(b_0,b_1,b_2)=(1,9,8)$ versus $(1,2,1)$, and we formally prove that our cellular message-passing scheme dynamically propagates this topological separation to distinguish these structures in the representation space. To validate topological awareness, we implement an $H_1$ Homology Ablation protocol: destroying cycle-forming 2-cells yields a $7.06\times$ sensitivity ratio in global Euclidean representations compared to non-topological perturbations, proving the model exhibits reduced representation stability when topological structure is severed. Finally, we prove that cellular extraction runs in $\mathcal{O}(|E|+|F|)$ time under a bounded-2-cell-size condition made explicit in Section 3.3, and achieve a 71.05\% zero-shot cross-domain transfer accuracy (PROTEINS to NCI1), outperforming GCN baselines by a 21.10-point margin.
...
Edges aggregate signals from their boundary 0-cells and their co-boundary 2-cells globally. The feature update matrix $M_E$ utilizes the absolute (unoriented) boundary matrices to ensure permutation equivariance:
\begin{equation}
...
Faces (2-cells) similarly aggregate signals from their bounding 1-cells (edges) using the transposed absolute edge-to-face mapping $|B_2|^T$:
\begin{equation}
    M_F^{(t+1)} = |B_2|^T H_E^{(t)}
\end{equation}
\begin{equation}
...
\emph{Note on Permutation Equivariance:} We explicitly pass the absolute (unoriented) incidence matrices $|B_1|$ and $|B_2|$ to ensure strict permutation equivariance. Using oriented boundary matrices with lexicographical ordering breaks permutation symmetry by relying on arbitrary vertex IDs. By utilizing the absolute incidence matrices, our Cellular Message Passing layers do not strictly preserve $B_1 B_2 = 0$ homological cancellation; instead, they function as a permutation-equivariant, topologically-motivated GCN. The absolute matrices provide a powerful inductive structural bias that enables the network to actively count face and edge participations without violating permutation invariance.
...
class CellularMessagePassingLayer(nn.Module):
    ...
    def forward(self, x_0, x_1, x_2, B1, B2):
        # 0. Enforce Permutation Equivariance via Absolute Matrices
        abs_B1 = torch.abs(B1)
        abs_B2 = torch.abs(B2)
        
        # 1. Edge Aggregation (Nodes -> Edges AND Faces -> Edges)
        msg_from_nodes = torch.sparse.mm(abs_B1.t(), x_0)
        msg_from_faces = torch.sparse.mm(abs_B2, x_2)
        m_edge = msg_from_nodes + msg_from_faces
        x_1_new = self.mlp_edge(x_1 + m_edge)
        
        # 2. Face Aggregation (Edges -> Faces)
        msg_from_edges = torch.sparse.mm(abs_B2.t(), x_1)
...
With latent dimension $D \ge |V|+|E|+|F|$, the cellular sum-decomposition readout is injective on the cellular multiset, resolving the isomorphism of SRG(16,6,2,2) pairs. \textbf{Limitation Statement:} This injectivity result is bounded by the latent dimension $D$ and serves as a local proof of expressivity for small SRG classes, rather than a proof of global isomorphism resolution for arbitrary graphs.
\end{theorem}
\begin{proof}
...
\begin{itemize}
    ...
    \item \textbf{DynamicCW (Cellular Message Passing):} Control Shift = $0.2607$, Topological Shift = $1.8405$. \textbf{Sensitivity Ratio: 7.06x}.
\end{itemize}
The destruction of the 2-cell triggers a massive, disproportionate state shift, yielding a $7.06\times$ sensitivity ratio. This confirms that the model's representations are highly responsive to topological changes, proving that the model exhibits reduced representation stability when topological structure is severed, providing strong empirical evidence of our topologically-motivated inductive bias.
